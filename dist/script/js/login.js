							...
							if(data) {
								...
								require(['jqcookie'], function() {
									...
									setTimeout(function() {
										...
										window.location.href = 'http://10.31.155.148/dangdang--/dist/index.html'
									}, 10)
								})

							} else { //用户名密码不正确

								$tip.eq(0).html('请输入您输入的用户名不存在 <a href="http://10.31.155.148/dangdang--/dist/registor.html">免费注册</a>').css('color', 'red')
								$user.val('')
								$pass.val('')
							}
						})
